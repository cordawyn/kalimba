...
or rather, design pattern. Basically, the steps of designing your Kalimba models
are as follows:
...
1) define the base module(s), roughly corresponding to RDFS classes
    ...
    module RDFS::Human
      ...
      # define base URI for the instances of this RDFS class
      base_uri "http://example.org/humans"
    ...
    module RDFS::Engineer
      ...
      # define base URI for the instances of this RDFS class
      base_uri "http://example.org/engineers"
    ...
    # your model may "inherit" multiple RDFS classes
    module RDFS::Person
      extend Kalimba::RDFSClass
...
2) from this point on, you may treat your RDFS class just like
any fully-fledged clone of ActiveModel (i.e. ActiveRecord model)

    $ alice = RDFS::Person.new(:name => "Alice")
    $ alice.valid?
    $ alice.save!
    ...

For details refer to YARD documentation for Kalimba::RDFSClass module.
